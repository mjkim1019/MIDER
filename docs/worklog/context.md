...
| 2026-04-24 | **T71 리뷰 반영 — 5건 수정**: (C1) HARDCODED_SECRET/DB_URL 전체 매치에 _mask_center 시 값 노출 → 전체 strong mask로 통일 (C2) 겹치는 스팬 str.replace 누락 → 스팬 dedup + 우측→좌측 슬라이스 치환 (H1) 로마자 이름 2자/영단어 성씨 제거로 Min_Size/No_Error 등 식별자 오탐 방지 + snake_case 구분자 제거 (H2) 세션 재인증 후 return 직행 → continue로 변경해 PII 재시도 경로 보존 (M3) 외국인등록번호 dead code 해결(주민번호 앞으로 이동) | 리뷰에서 CRITICAL 2건/HIGH 3건/MEDIUM 1건 발견. 마스킹이 T71 목적을 무력화하는 치명적 결함 → 전체 마스킹 원칙으로 전환. LLM은 "[PII]" 구조 신호만으로 분석 가능, 실제 값 불필요. 109개 T71 테스트 통과 (회귀 13건 추가) |
| 2026-04-23 | **T73.0 신규 추가 — ProFrame 공통 헤더 구조 추출 + 번들**: pfm*.h 7종(2,351줄)을 원본 번들하지 않고 구조만 추출하여 2종 산출 (`pfm.slim.h` clang-tidy용 + `pfm_symbols.yaml` LLM 프롬프트용). 원본은 .gitignore 유지, 산출물만 커밋. 버전 해시 주석으로 stale 방지. `include_resolver`가 기본 헤더 DB로 사용 → T73.3 인터랙션 트리거는 pfm 외 사내 커스텀 헤더에만 발동 | 사용자가 `mider/resources/headers/`에 원본 pfm*.h 7개 배치 → 검토 결과 원본 번들은 (1) SKT 기밀 유출 (2) 프롬프트 토큰 5~8배 증가 (3) 주석/히스토리 noise의 3중 문제. 벡터 임베딩 RAG(T63) 대안도 검토했으나 exact-symbol-lookup 본질 + 폐쇄망 AICA 의존 + 비결정론 사유로 기각. 구조 추출이 정확도/비용/보안 모두 우위 |
| 2026-04-23 | **T73.0.1 파서 libclang 확정**: libclang vs tree-sitter vs regex 3종 비교 후 libclang 채택. 빌드 타임 1회 추출 도구라 바이너리 크기 제약 무관하므로 정확도가 유일 기준. `#define X (Y*2)` 중첩 매크로 자동 평가 + 헤더 간 `-I` 경로로 batch 파싱 + typedef/구조체 크기 정확 계산이 결정적. 7개 헤더를 `mider/resources/headers/`를 include path로 묶어 한 번에 파싱. tree-sitter는 2순위 fallback, regex는 커버리지 리스크로 제외 | pfm 헤더의 버퍼 크기 상수가 LLM OOB 판단의 핵심 정보. 매크로 값 누락 시 T73 존재 의미 무너짐. libclang만 실제 컴파일러 프론트엔드 기반이라 구조적 누락 위험 없음. `pip install libclang`으로 OS별 prebuilt wheel 제공되어 SKT 개발 머신 설치 장벽 낮음 |
| 2026-04-24 | **T64 완료 (리소스 경로 레이어)**: `resource_path.py` 신설 — 3단계 우선순위(환경변수 > exe옆 > 번들). `prompt_loader.py` 리팩토링(PROMPTS_DIR alias 유지), `mider.spec`에 rules/skills 번들 블록 추가, `scripts/export_default_resources.py` 스크립트, `docs/dev_vs_prod.md` 운영 가이드 작성 | T65(Skills 로더)의 선결 조건. 운영기에서 번들 기본값을 유지하면서 exe 옆 drop-in 커스터마이징 지원. 사용자 요청으로 개발기/운영기 차이 가이드 동반 작성. T59 서명 기능은 P2 잔존 과제 |
| 2026-04-24 | **T70 완료 (Reporter 속도 개선)**: 프롬프트 140줄→25줄 축소(risk_description 전용), LOW 위험도(critical=0 AND high=0) LLM 호출 skip, 단계별 프로파일링 인스트루먼테이션. 사용자 추가로 T70.6.1/T70.6.2 배포 판정 정책 개편 포함 | 20초 병목 원인이 "LLM이 전체 리포트 JSON 생성 요구, 코드는 risk_description만 추출"하는 구조적 낭비였음. 프롬프트 축소로 90%+ 토큰 감소, LOW 케이스는 LLM 자체 skip으로 0초 응답. T72 완료 후 병렬 LLM 호출(T70.5) 재평가 예정 |
| 2026-04-30 | **T72.0 신규 추가 — 단기 우회: LLM 출처 medium suggestion 필터 (P0)**: medium severity 자체 제거 안 함, `source=='llm' AND severity=='medium'` 조합만 reporter primary/터미널에서 제외. JSON에는 보존. T72 본 작업 완료 시 issue_type 필드로 대체하며 제거 (TODO 주석 명시). plan/checklist 모두 원칙 명시: medium은 영향도 직교 축으로 유지 | 사용자 요청 "지금 medium 너무 많고 개선 제안도 같이 나옴". medium 일률 삭제 안은 scanner 출처 medium(pid_scanner/secret_scanner/embedded_sql_analyzer/proc_cross_checker/explain_plan_parser)의 보안·SQL 진짜 이슈를 같이 잃기 때문에 기각. Issue 스키마에 이미 `source` 필드(static_analysis/llm/hybrid)가 있어 LLM 출처만 핀포인트 필터 가능. 본질은 issue_type 분리(T72)이지만 작업량 크므로 단계적 진행 |
