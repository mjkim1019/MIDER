...
### T52: CLI 로그인 UX + 설정 업데이트 (depends: T51)
- T52.1: `main.py` — 인터랙티브 `login` 명령어 + 시작 시 세션 체크 → `mider/main.py`
  - 인터랙티브 프롬프트에서 `login` 입력 시 SSO 로그인 실행
  - AICA provider 시작 시 세션 유효성 체크 → 없으면 "login을 입력하세요" 안내
  - `--sso` 플래그 제거 (인터랙티브 login으로 대체)
...
| **인터랙티브 `login` 명령어** | `--sso` 플래그 대신 프롬프트에서 `login` 입력 — 직관적 UX |
| **시작 시 SSO 세션 자동 체크** | AICA provider에서 세션 없으면 "login을 입력하세요" 안내 — 사용자가 원인을 바로 파악 |
