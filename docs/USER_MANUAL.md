...
1. 명령 프롬프트(cmd)를 열고 `mider.exe`가 있는 폴더로 이동합니다.
2. 분석할 파일의 경로를 지정하여 실행합니다:

```bash
mider.exe -f 파일경로1 파일경로2
```
...
```bash
# 단일 파일 분석
mider.exe -f C:\src\ordsb0100010t01.c

# 여러 파일 동시 분석
mider.exe -f C:\src\ordsb0100010t01.c C:\src\zinvbpre01140.pc C:\src\zord_svc_f101.sql
...
2. 절대경로를 사용해보세요: `mider.exe -f C:\src\app.c`
3. 상대경로는 현재 작업 디렉토리 기준입니다.
...
2. `--output` 옵션으로 다른 출력 경로를 지정해봅니다: `mider.exe -f app.c -o C:\결과`
