...
- T53.1: `resolve_input_files()` 수정 → `mider/main.py`
  - `input/` 폴더 탐색 유지 (기존 호환)
  - `base_dir` 하위 전체 재귀 검색(`rglob`) 단계 추가 (input에서 못 찾을 때)
  - 동일 파일명 여러 개 발견 시 목록 표시 + 에러 처리
  - 검색 순서: 절대경로 → CWD 상대경로 → input 폴더 → base_dir 하위 재귀 검색
...
| **`input/` 폴더 유지 + `base_dir` rglob 검색 추가** | A: input 복사 방식(mider 위치 무관), B: workspace 재귀 검색(복사 불필요) — 두 배포 시나리오 모두 지원 |
