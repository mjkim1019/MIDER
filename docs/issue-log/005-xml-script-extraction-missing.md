...
### Phase 3: 중복 ID 스코프 개선 ✅ (T25, 2026-03-17)
- 문제: 전체 XML에서 중복 검출 → 서로 다른 dataList 간 동명 컬럼도 중복 보고
- 해결: `_extract_component_ids`에서 `column`/`columnInfo`/`data` 태그를 제외
- `dataList`/`dataMap` ID는 `$w.getById()`로 접근하는 document-level ID이므로 유지
- 제외 태그를 `_DATA_DEFINITION_TAGS` frozenset 상수로 관리
